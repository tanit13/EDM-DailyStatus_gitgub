...
{
    public partial class FormStreamKexy : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                try
                {
                    ...
                    txt_StreamName_popup.Text = Request.QueryString["id"].ToString();
                ...
                (Exception ex)
                {
                    MsgAlert = "Cannot get data: " + txt_StreamName_popup.Text.Trim().ToUpper() + ", Bacause" + ex.Message.ToString();
        ...
        protected void GetEDMDataforTable()
        {
            ...
            try
            {
                string EDMGroup_id = txt_StreamName_popup.Text.Split('.')[0];
                EDMGroup_id.Replace("_", " ");
                string TierID = txt_StreamName_popup.Text.Split('.')[1];
                ...
                lt_GroupTableSpecialStatus.Text = strEDMStatus;
                lt_GroupTableSpecialStatus.Text = strSpecialStatus;

                if (intDataStatus > 0)
                {
                    iStatusCountEDM.InnerHtml = intDataStatus.ToString();
                }
                else
                {
                    iStatusCountEDM.InnerHtml = "";
                }

                if (intSpecialStatus > 0)
                {
                    iSpecialCountStatus.InnerHtml = intSpecialStatus.ToString();
                }
                else
                {
                    iSpecialCountStatus.InnerHtml = "";
