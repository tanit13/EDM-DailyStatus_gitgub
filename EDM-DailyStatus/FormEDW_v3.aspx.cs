...
{
    public partial class FormEDW_v3 : System.Web.UI.Page
    {
        ...
        public void getSumStatusEDW()
        {
            ...
            //  div_num_completed.InnerText = lst[0].Completed.ToString();
               ...
               // h_num_meet.InnerText = lst[0].Meet.ToString();
                ...
                StringBuilder str_dataMorrisPieDonut = new StringBuilder();
                StringBuilder js_json = new StringBuilder();
                ...
                // -- int data morris pie donut chart ---
                str_dataMorrisPieDonut.Append("<script>");
                str_dataMorrisPieDonut.Append(" var vaDataMrrisDonut = ").Append(lst[0].Completed_percent.ToString());
                str_dataMorrisPieDonut.Append("</script>");

                h_total_on_pie.InnerText = lst[0].Total.ToString();
                b_completed_job.InnerText = lst[0].Completed.ToString();


                //lt_dataPieChart.Text = js_json.ToString();
                lt_dataPieChart.Text = str_dataMorrisPieDonut.ToString();

                // ----- set value to pie chart  ------
                //span_bar_circle.InnerText = lst[0].Completed_percent.ToString();
                string str_comple_p = "";
                if (lst[0].Completed_percent < 51)
                {
                    str_comple_p = "progress-circle  p" + lst[0].Completed_percent;
                }
                else {
                    str_comple_p = "progress-circle over50  p" + lst[0].Completed_percent;
                }
                //div_bar_circle.Attributes.Add("class", str_comple_p);

                //h_total_on_pie.InnerText = lst[0].Total.ToString();
                //h_complete_on_pie.InnerText = lst[0].Completed_percent.ToString() + "%";
                //h_running_on_pie.InnerText = lst[0].Running_percent.ToString() + "%"; ;
                //h_monitor_on_pie.InnerText = lst[0].Monitor_percent.ToString() + "%"; ;
        ...
        private void getDataMorrisLineChartSum()
        {
            ...
            lst_obj = oratDao.dataMorrisLineChartSumEDW();
        ...
        public static string getDataMorrisLineChart()
        {
            ...
            str_data = new JavaScriptSerializer().Serialize(oratDao.dataMorrisLineChartEDW());
